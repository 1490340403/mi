      ...
      <div class="container">
        <div class="cart-box">
          ...
          <ul class="cart-item-list">
            <li class="cart-item" v-for="(item,index) in cartProductVoList" :key="index">
...
export default {
    ...
    methods:{
        ...
        datas(res){
            ...
            let total=0
            res.cartProductVoList.forEach(element => {
                if(element.productSelected==true){
                    total+=element.quantity
                }
            });
           
            this.selectNum=total
        },
        desc(item){
            if(item.quantity==1){
                alert('数量最少为1')
        ...
        updataNum(item){
            this.axios.put(`/carts/${item.productId}`,{quantity:item.quantity,selected:true}).then(res=>{
                this.datas(res)
            })
        },
        changeChecked(item){
            this.axios.put(`/carts/${item.productId}`,{quantity:item.quantity,selected:!item.productSelected}).then(res=>{
                this.datas(res)
            })
        },
        order(){
        ...
        toggleAll(){
            let {selectedAll}=this
            let url=selectedAll==true?'/carts/unSelectAll':'/carts/selectAll'
